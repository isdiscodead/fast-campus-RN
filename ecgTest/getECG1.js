...
let ecg_data = _.map(data, row => parseFloat(row[0])).filter((data) => !isNaN(data)); // NaN 데이터 삭제
...
let ecg_std = Math.sqrt(devTotal/ecg_data.length).toFixed(5);   /* 모표준편차값 */
let ecg_sstd = Math.sqrt(devTotal/(ecg_data.length-1));


// 이상치 제거 (5σ 범위 밖)
ecg_data = _.filter(ecg_data, x => Math.abs(x - ecg_mean) < 5 * ecg_std);


// R-peak 찾기 (간단한 알고리즘 사용)
let r_peaks_locs = [];
for(let i=1; i<ecg_data.length-1; i++) {
    if(ecg_data[i] > 0.4 && ecg_data[i] > ecg_data[i-1] && ecg_data[i] > ecg_data[i+1]) {
        if (i >= 60 && r_peaks_locs.length === 0 || i - r_peaks_locs[r_peaks_locs.length - 1] >= 150) {
...
// RMSSD 값 계산
let diff_rr_intervals_sqrd_summed_ms2=_.sum(_.map(rr_intervals_ms,(x,i,arr)=>i>0?Math.pow(x-arr[i-1],2):0));
let rmssd_val_ms=Math.sqrt(diff_rr_intervals_sqrd_summed_ms2/(rr_intervals_ms.length - 1));
